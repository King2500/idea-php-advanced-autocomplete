...
public class PhpCompletionTokens {
    ...
    public final static HashMap<String, String> httpHeaderFieldsDocumentation = new HashMap<String, String>() {{
        put("Accept", "The <b><code>Accept</code></b> request HTTP header advertises which content types, expressed as MIME types, the client is able to understand. Using content negotiation, the server then selects one of the proposals, uses it and informs the client of its choice with the Content-Type response header. Browsers set adequate values for this header depending on the context where the request is done: when fetching a CSS stylesheet a different value is set for the request than when fetching an image, video or a script.");
        put("Accept-Charset", "The <b><code>Accept-Charset</code></b> request HTTP header advertises which character set the client is able to understand. Using content negotiation, the server then selects one of the proposals, uses it and informs the client of its choice within the Content-Type response header. Browsers usually don't set this header as the default value for each content type is usually correct and transmitting it would allow easier fingerprinting.<br><br>If the server cannot serve any matching character set, it can theoretically send back a 406 (Not Acceptable) error code. But, for a better user experience, this is rarely done and the more common way is to ignore the Accept-Charset header in this case.");
        put("Accept-Encoding", "<p>The <b><code>Accept-Encoding</code></b> request HTTP header advertises which content encoding, usually a compression algorithm, the client is able to understand. Using content negotiation, the server selects one of the proposals, uses it and informs the client of its choice with the <code>Content-Encoding</code> response header.</p>\n" +
                ...
                "</ul>" +
                " <p>As long as the <code>identity</code> value, meaning no encoding, is not explicitly forbidden, by an <code>identity;q=0</code> or a <code>*;q=0</code> without another explicitly set value for identity, the server must never send back a <code>406 Not Acceptable</code> error.</p>");
        put("Accept-Language", "<p>The <b><code>Accept-Language</code></b> request HTTP header advertises which languages the client is able to understand, and which locale variant is preferred. (By languages, we mean natural languages, such as English, and not programming languages.) Using content negotiation, the server then selects one of the proposals, uses it and informs the client of its choice with the <code>Content-Language</code> response header. Browsers set adequate values for this header according to their user interface language and even if a user can change it, this happens rarely (and is frowned upon as it leads to fingerprinting).</p>" +
                ...
                "<p>If the server cannot serve any matching language, it can theoretically send back a <code>406</code> (Not Acceptable) error code. But, for a better user experience, this is rarely done and more common way is to ignore the <code>Accept-Language</code> header in this case.</p>");
        put("Accept-Ranges", "<p>The <b><code>Accept-Ranges</code></b> response HTTP header is a marker used by the server to advertise its support of partial requests. The value of this field indicates the unit that can be used to define a range.</p>\n" +
                "<p>In presence of an <code>Accept-Ranges</code> header, the browser may try to <i>resume</i> an interrupted download, rather than to start it from the start again.</p>");
        put("Access-Control-Allow-Credentials", "<p>The <b><code>Access-Control-Allow-Credentials</code></b> response header tells browsers whether to expose the response to frontend JavaScript code when the request's credentials mode (<code>Request.credentials</code>) is \"<code>include</code>\".</p>\n" +
                "<p>When a request's credentials mode (<code>Request.credentials</code>) is \"<code>include</code>\", browsers will only expose the response to frontend JavaScript code if the <code>Access-Control-Allow-Credentials</code> value is <code>true</code>.</p>\n" +
                ...
                "<p>The <code>Access-Control-Allow-Credentials</code> header works in conjunction with the <code>XMLHttpRequest.withCredentials</code> property or with the <code>credentials</code> option in the <code>Request()</code> constructor of the Fetch API. For a CORS request with credentials, in order for browsers to expose the response to frontend JavaScript code, both the server (using the <code>Access-Control-Allow-Credentials</code> header) and the client (by setting the credentials mode for the XHR, Fetch, or Ajax request) must indicate that they’re opting in to including credentials.</p>");
        put("Access-Control-Allow-Headers", "<p>The <b><code>Access-Control-Allow-Headers</code></b> response header is used in response to a preflight request which includes the <code>Access-Control-Request-Headers</code> to indicate which HTTP headers can be used during the actual request.</p>\n" +
                "<p>The simple headers, <code>Accept</code>, <code>Accept-Language</code>, <code>Content-Language</code>, <code>Content-Type</code> (but only with a MIME type of its parsed value (ignoring parameters) of either <code>application/x-www-form-urlencoded</code>, <code>multipart/form-data</code>, or <code>text/plain</code>), are always available and don't need to be listed by this header.</p>\n" +
                "<p>This header is required if the request has an <code>Access-Control-Request-Headers</code> header.</p>");
        put("Access-Control-Allow-Methods", "<p>The <b><code>Access-Control-Allow-Methods</code></b> response header specifies the method or methods allowed when accessing the resource in response to a preflight request.</p>");
        put("Access-Control-Allow-Origin", "<p>The <b><code>Access-Control-Allow-Origin</code></b> response header indicates whether the response can be shared with requesting code from the given origin.</p>");
        put("Access-Control-Expose-Headers", "<p>The <b><code>Access-Control-Expose-Headers</code></b> response header indicates which headers can be exposed as part of the response by listing their names.</p>\n" +
                "<p>By default, only the 6 simple response headers are exposed:</p>\n" +
                "<ul>" +
        ...
        put("Access-Control-Max-Age", "<p>The <b><code>Access-Control-Max-Age</code></b> response header indicates how long the results of a preflight request (that is the information contained in the <code>Access-Control-Allow-Methods</code> and <code>Access-Control-Allow-Headers</code> headers) can be cached.</p>");
        put("Access-Control-Request-Headers", "<p>The <b><code>Access-Control-Request-Headers</code></b> request header is used when issuing a preflight request to let the server know which HTTP headers the client might send when the actual request is made.</p>");
        put("Access-Control-Request-Method", "<p>The <b><code>Access-Control-Request-Method</code></b> request header is used when issuing a preflight request to let the server know which HTTP method will be used when the actual request is made. This header is necessary as the preflight request is always an <code>OPTIONS</code> and doesn't use the same method as the actual request.</p>");
        put("Age", "<p>The <b><code>Age</code></b> header contains the time in seconds the object has been in a proxy cache.</p>\n" +
                "<p>The <code>Age</code> header is usually close to zero. If it is <code>Age: 0</code>, it was probably just fetched from the origin server; otherwise It is usually calculated as a difference between the proxy's current date and the <code>Date</code> general header included in the HTTP response.</p>");
        put("Allow", "<p>The <b><code>Allow</code></b> header lists the set of methods support by a resource.</p>\n" +
                "<p>This header must be sent if the server responds with a <code>405 Method Not Allowed</code> status code to indicate which request methods can be used. An empty <code>Allow</code> header indicates that the resource allows no request methods, which might occur temporarily for a given resource, for example.</p>");
        put("Authorization", "<p>The HTTP <b><code>Authorization</code></b> request header contains the credentials to authenticate a user agent with a server, usually after the server has responded with a <code>401 Unauthorized</code> status and the <code>WWW-Authenticate</code> header.</p>");
        put("Cache-Control", "<p>The <b><code>Cache-Control</code></b> general-header field is used to specify directives for caching mechanisms in both requests and responses. Caching directives are unidirectional, meaning that a given directive in a request is not implying that the same directive is to be given in the response.</p>");
        put("Clear-Site-Data", "<p>The <b><code>Clear-Site-Data</code></b> header clears browsing data (cookies, storage, cache) associated with the requesting website. It allows web developers to have more control over the data stored locally by a browser for their origins.</p>");
        put("Connection", "<p>The <b><code>Connection</code></b> general header controls whether or not the network connection stays open after the current transaction finishes. If the value sent is <code>keep-alive</code>, the connection is persistent and not closed, allowing for subsequent requests to the same server to be done.</p>\n" +
                "<p>Connection-specific header fields such as <code>Connection</code> must not be used with HTTP/2.</p>\n" +
                "<p>Except for the standard hop-by-hop headers (<code>Keep-Alive</code>, <code>Transfer-Encoding</code>, <code>TE</code>, <code>Connection</code>, <code>Trailer</code>, <code>Upgrade</code>, <code>Proxy-Authorization</code> and <code>Proxy-Authenticate</code>), any hop-by-hop headers used by the message must be listed in the <code>Connection</code> header, so that the first proxy knows it has to consume them and not forward them further. Standard hop-by-hop headers can be listed too (it is often the case of <code>Keep-Alive</code>, but this is not mandatory).</p>");
